...
def download_handler(utorid, quercus_id):
    base = f"Getting submission for {utorid}..."
    ...
    if submission_data is None:
        print(f"{base} [ERROR] User not found.", flush=True)
    elif submission_data['missing']:
        print(f"{base} [MISSING] Skipping...", flush=True)
    elif submission_data['late']:
        print(f"{base} [LATE] Skipping...", flush=True)
    else:    
        os.makedirs(f'{candidates_dir}/{utorid}', exist_ok=True)
        file_path = f'{candidates_dir}/{utorid}/{file_name}'
        
        done = quercus.download_submission_file(
            token=token, 
...
fs = [executor.submit(download_handler, u, q) for u, q in mapping.items()]
