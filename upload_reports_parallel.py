...
def report_upload_handler(student_dir):
    ...
    if student_dir not in mapping:
        ...
        base = f"- Uploading {student_dir} report ... "
        print(base + ("Done." if done else "[ERROR] Failed."), flush=True)

student_dir_list = os.listdir(assgn_dir + '/candidates')

executor = concurrent.futures.ProcessPoolExecutor(20)
fs = [executor.submit(report_upload_handler, sd) for sd in student_dir_list]
